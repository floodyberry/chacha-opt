* `gcc` is unable to track modified assembler files included by the `as` macros due to the acrobatics required to gather files included by `gcc` (with the C preprocessor) and `as` (with the `as macros`).
